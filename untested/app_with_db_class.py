...
def upload_file():
    if request.method == "POST":
        ...
        md5_key = db.db_make_md5_hash("/var/www/scans/malware_file")
        if db.db_init()[0] != []:
            if md5_key in db.db_init()[0]:
...
def get_json():
    if "md5_hash" in request.args:
        if request.args["md5_hash"] in db.db_init()[0]:
            return db.db_get_entry(request.args["md5_hash"])
        else:
...
def scan_file(malware="malware_file"):
    ...
    db.db_make_entry(fp, json_results)
